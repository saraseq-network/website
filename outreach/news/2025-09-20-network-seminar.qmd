...
> Each row represents a directed connection from an origin premises to a destination premises. Along with the connection itself, we summarize two key attributes: 
**shipments** = the total number of movements between that pair of premises; **animals** = the total number of head(s) moved across all those shipments
...
#### **1.2. We can also explore **In-Degree** and **Out-Degree** seperately!**
...
in_out_plot <- par(mfrow = c(1,2), mar = c(1,1,3,1))
